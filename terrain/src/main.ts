...
import { ComponentStore, Entity, World } from "./ecs.js";
...
const FISH_MARKET_CONFIG: MarketCommodityConfig = {
  basePrice: 10,
  minPrice: 1,
  maxPrice: 100,
  elasticity: 1
...
type Phase = "map_edit" | "settlement" | "simulation";
type Brush = "grass" | "water" | "tree";
type SettlementTab = "government" | "population" | "resources" | "labor" | "buildings";
...
const TERRAIN_COLORS: Record<TerrainCode, string> = {
...
const positions = new ComponentStore<TilePosition>();
const buildings = new ComponentStore<Building>();
const settlements = new ComponentStore<Settlement>();
const renderables = new ComponentStore<Renderable>();
const buildingByPixel = new Map<string, Entity>();
...
const brushButtons = Array.from(document.querySelectorAll<HTMLButtonElement>(".brush-btn[data-brush]"));
...
const settlementTabButtons = Array.from(
  document.querySelectorAll<HTMLButtonElement>(".settlement-tab-btn[data-tab]")
);
const settlementPanels = Array.from(document.querySelectorAll<HTMLElement>(".settlement-panel[data-tab-panel]"));
...
let phase: Phase = "map_edit";
let selectedBrush: Brush = "grass";
let brushSize = Math.max(2, Number.parseInt(brushSizeInput.value, 10) || 12);
let hoveredPixel: TilePosition | null = null;
let isPainting = false;
let lastPaintPixel: TilePosition | null = null;
let firstSettlementEntity: Entity | null = null;
let selectedSettlementEntity: Entity | null = null;
let activeSettlementTab: SettlementTab = "government";
let tickCount = 0;
let dayCount = 0;
...
const pixelKey = (pixel: TilePosition): string => `${pixel.x},${pixel.y}`;

const isBrush = (value: string): value is Brush => {
  return value === "grass" || value === "water" || value === "tree";
};

const isSettlementTab = (value: string): value is SettlementTab => {
  return value === "government" || value === "population" || value === "resources" || value === "labor" || value === "buildings";
};

const parseSpeedMultiplier = (value: string): number => {
...
const probabilisticRound = (value: number): number => {
  if (value <= 0) {
    return 0;
  }
...
const formatSpeedMultiplier = (): string => `${simulationSpeedMultiplier}x`;

const currentTickIntervalMs = (): number => {
  return Math.max(16, Math.round(TICK_BASE_MS / simulationSpeedMultiplier));
};

const terrainCodeForBrush = (brush: Brush): TerrainCode => {
  if (brush === "water") {
...
const isInsideMap = (pixel: TilePosition): boolean => {
  return pixel.x >= 0 && pixel.x < MAP_WIDTH && pixel.y >= 0 && pixel.y < MAP_HEIGHT;
};

const terrainAt = (pixel: TilePosition): TerrainCode => {
  return terrainData[(pixel.y * MAP_WIDTH) + pixel.x] as TerrainCode;
};

const fillTerrainBackground = (): void => {
  terrainCtx.fillStyle = TERRAIN_COLORS[TerrainCode.Grass];
  terrainCtx.fillRect(0, 0, MAP_WIDTH, MAP_HEIGHT);
};

const refreshTerrainTexture = (): void => {
  terrainSprite.texture.baseTexture.update();
};

const setTerrainPixel = (x: number, y: number, nextCode: TerrainCode): boolean => {
  if (x < 0 || x >= MAP_WIDTH || y < 0 || y >= MAP_HEIGHT) {
    return false;
  }

  const index = (y * MAP_WIDTH) + x;
  const previousCode = terrainData[index] as TerrainCode;
...
const clearSettlementDetailValues = (): void => {
...
const setActiveSettlementTab = (tab: SettlementTab): void => {
...
const showSettlementInfo = (entity: Entity | null): void => {
...
const drawHoverPixel = (pixel: TilePosition | null): void => {
  hoverGraphics.clear();
  if (!pixel) {
    return;
  }
...
const pointerToPixel = (event: PixiPointerEvent): TilePosition | null => {
...
const drawSettlementSprite = (sprite: PixiGraphics, pixel: TilePosition, borderRadius: number): void => {
  sprite.clear();
...
const createBuildingSprite = (pixel: TilePosition, borderRadius: number): PixiGraphics => {
  const sprite = new PIXI.Graphics();
  drawSettlementSprite(sprite, pixel, borderRadius);
  return sprite;
};

const refreshSettlementSprite = (entity: Entity): void => {
  const settlement = settlements.get(entity);
  const position = positions.get(entity);
...
const updateBrushSizeLabel = (): void => {
  brushSizeValue.textContent = `${brushSize}px`;
};

const updateControlsForPhase = (): void => {
...
const setPhase = (nextPhase: Phase): void => {
  phase = nextPhase;
...
const setBrush = (brush: Brush): void => {
  selectedBrush = brush;
  updateBrushButtons();
  drawHoverPixel(hoveredPixel);
};

const paintBrushAtPixel = (center: TilePosition): number => {
...
const paintStroke = (fromPixel: TilePosition, toPixel: TilePosition): number => {
  let changedPixels = 0;
...
const getBuildingPlacementIssue = (pixel: TilePosition): string | null => {
...
const syncLaborAndPopulationFromCohorts = (settlement: Settlement): void => {
  let hunters = 0;
  let gatherers = 0;
  let rulers = 0;
...
const updateSettlementRadius = (settlement: Settlement): void => {
...
const removePopulationFromLabor = (settlement: Settlement, laborType: LaborType, count: number): number => {
  let remaining = count;
...
const addPopulationToLabor = (settlement: Settlement, laborType: LaborType, count: number): void => {
  if (count <= 0) {
    return;
  }

  const existing = settlement.populationCohorts.find(
    (cohort) => cohort.cultureId === "A" && cohort.religionId === "A" && cohort.laborType === laborType
  );
...
const placeBuilding = (pixel: TilePosition): boolean => {
  ...
  const initialCohorts: PopulationCohort[] = [
    {
      cultureId: "A",
      cultureName: "Caananite",
      religionId: "A",
...
const findSettlementFromPixel = (pixel: TilePosition): Entity | null => {
...
const totalStoredResources = (settlement: Settlement): number => {
...
const clampSettlementToStorageCapacity = (settlement: Settlement): void => {
  ...
  const naturalTrimOrder: Array<keyof NaturalResources> = ["fish", "fruits", "wheat", "wood", "stone"];
  ...
  const manufacturedTrimOrder: Array<keyof Settlement["manufacturedResources"]> =
    ["charcoal", "cordage", "woodTools", "stoneTools"];
...
interface MarketBuyOrderInput {
  orderId: string;
  label: string;
  quantityRequested: number;
}

interface MarketDayResult {
  ...
  price: number;
  soldQuantity: number;
  carryoverInventory: number;
  orders: MarketCommodityOrder[];
}

const buildFishBuyOrders = (settlement: Settlement): MarketBuyOrderInput[] => {
  const orders: MarketBuyOrderInput[] = settlement.populationCohorts
...
const runCommodityMarketDay = (
  inventoryCarryover: number,
  producedToday: number,
  buyOrders: MarketBuyOrderInput[],
  ...
  const orders: MarketCommodityOrder[] = buyOrders.map((order) => {
...
const applyFishMarketForSettlement = (settlement: Settlement, fishProducedToday: number): number => {
  const fishState = settlement.market.fish;
  const marketResult = runCommodityMarketDay(
    fishState.inventory,
    fishProducedToday,
    buildFishBuyOrders(settlement),
    fishState.config
  );
...
const consumeSmokingHutWood = (settlement: Settlement): void => {
...
const applyDailyFishSpoilage = (settlement: Settlement): void => {
...
const applyWeeklyPopulationVitalRates = (settlement: Settlement): void => {
  for (const cohort of settlement.populationCohorts) {
...
const calculateDailyResourceGain = (workers: number, terrainTilesInRange: number): number => {
  const randomFactor = randomInRange(0.9, 1.1);
  return Math.max(0, workers * terrainTilesInRange * 0.001 * randomFactor);
};

const applyFoodShortageLosses = (settlement: Settlement, shortage: number): void => {
...
const runSimulationStep = (): void => {
  tickCount += 1;
  dayCount = tickCount;
  const governorRotationMessages: string[] = [];
  ...
  for (const [entity, settlement] of settlements.entries()) {
    ...
    if (dayCount >= settlement.nextGovernorSelectionDay) {
      ...
      governorRotationMessages.push(
        `Settlement (${position.x}, ${position.y}) elected ${settlement.tribalGovernorName}.`
      );
    }
  }
...
const beginSimulation = (): void => {
  ...
  setPhase("simulation");
  setStatus(
    `Simulation started. Each tick is one day at ${formatSpeedMultiplier()} speed (~${currentTickIntervalMs()}ms per day).`
  );
  startSimulationTimer();
};

const startPaintStroke = (pixel: TilePosition): void => {
  if (phase !== "map_edit") {
    return;
  }
...
const continuePaintStroke = (pixel: TilePosition): void => {
...
app.stage.on("pointermove", (event: PixiPointerEvent) => {
...
app.stage.on("pointerdown", (event: PixiPointerEvent) => {
  if (event.button !== 0) {
    return;
  }
  ...
  if (clickedSettlement !== null) {
    ...
    if (settlement) {
      setStatus(
        `Settlement selected. Governor: ${settlement.tribalGovernorName}. Population: ${settlement.population}, Food: ${formatAmount(settlement.totalFood)}, Wood: ${formatAmount(settlement.naturalResources.wood)}.`
      );
    }
